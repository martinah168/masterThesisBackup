...
from .blocks import *
from .nn import (adaptive_avg_pool_nd, conv_nd, linear, normalization,
...
class BeatGANsUNetModel(nn.Module):
    ...
    def __init__(self, conf: BeatGANsUNetConfig):
        ...
        for level, mult in enumerate(conf.input_channel_mult or
                                     conf.channel_mult):
            for _ in range(conf.num_input_res_blocks or conf.num_res_blocks):
                ...
                if resolution in conf.attention_resolutions:
                    layers.append(
                        AttentionBlock(
                            ch,
                            use_checkpoint=conf.use_checkpoint or
                            conf.attn_checkpoint,
                ...
                if level and i == conf.num_res_blocks:
                    ...
                    layers.append(
                        ResBlockConfig(
                            ...
                            up=True,
                            **kwargs,
                        ).make_model() if (conf.resblock_updown
                                          ) else Upsample(ch,
                                                          conf.conv_resample,
                                                          dims=conf.dims,
                                                          out_channels=out_ch))
