...
from utils.enums_model import GenerativeType, ModelMeanType, ModelType, ModelVarType, LossType
...
class Gaussian_Diffusion:
    ...
    def __init__(
        ...
        beta_scheduler: Beta_Schedule | None,
        timesteps: int,
        ...
        self.init(beta_scheduler, timesteps)

    def calc_betas(self, beta_scheduler: Beta_Schedule | None, timesteps: int):
        if beta_scheduler is None:
            betas = self.betas
        else:
            betas = get_named_beta_schedule(beta_scheduler, timesteps)
        ...
        self.betas = betas
        return self.betas, alphas, self.alphas_cumprod

    def init(self, beta_scheduler: Beta_Schedule | None, timesteps: int):
        # Use float64 for accuracy.
        betas, alphas, alphas_cumprod = self.calc_betas(beta_scheduler, timesteps)
    ...
    def training_losses(self, model: Model, x_start: torch.Tensor, t: torch.Tensor, model_kwargs=None, noise: torch.Tensor | None = None):
        """
        ...
        if self.loss_type in [LossType.mse, LossType.l1]:
            ...
            # get the pred xstart
            p_mean_var = self.p_mean_variance(model=DummyModel(pred=_model_output), x=x_t, t=t, clip_denoised=False)  # type: ignore
    ...
    def sample(self, model: Model, shape=None, noise=None, cond=None, x_start=None, clip_denoised=True, model_kwargs=None, progress=False):
        """
        Args:
        ...
        param = dict(shape=shape, noise=noise, clip_denoised=clip_denoised, model_kwargs=model_kwargs, progress=progress)
        if self.gen_type == GenerativeType.ddpm:
            return self.p_sample_loop(model, **param)
        elif self.gen_type == GenerativeType.ddim:
            return self.ddim_sample_loop(model, **param)
        else:
            raise NotImplementedError()
    ...
    def _predict_xstart_from_scaled_xstart(self, t, scaled_xstart):
        return scaled_xstart * _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, scaled_xstart.shape)
    ...
    def _predict_eps_from_scaled_xstart(self, x_t, t, scaled_xstart):
        ...
        return (x_t - scaled_xstart) / _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        ...
        for i in indices:
            ...
            with torch.no_grad():
                out = self.ddim_reverse_sample(
                    model, sample, t=t, clip_denoised=clip_denoised, denoised_fn=denoised_fn, model_kwargs=model_kwargs, eta=eta
                )
                sample = out["sample"]
                # [1, ..., T]
...
class SpacedDiffusion(Gaussian_Diffusion):
    """
...
def get_sampler(opt: DAE_Option, eval: bool = True, T=None) -> Gaussian_Diffusion:
    ...
    if opt.generative_type == GenerativeType.ddpm and not force_ddim:
        section_counts = [T]
    elif opt.generative_type == GenerativeType.ddim:
        section_counts = f"ddim{T}"
    else:
        raise NotImplementedError()

    return SpacedDiffusion(
        beta_scheduler=opt.beta_schedule,
        timesteps=opt.num_timesteps,
        gen_type=opt.generative_type,
        model_type=opt.model_type,
        model_mean_type=opt.model_mean_type,
        model_var_type=opt.model_var_type,
        loss_type=opt.loss_type,
        rescale_timesteps=opt.rescale_timesteps,
        use_timesteps=space_timesteps(num_timesteps=opt.num_timesteps, section_counts=section_counts),
        fp16=not opt.fp32,
    )
...
class _WrappedModel:
    ...
    def forward(self, x, t, t_cond=None, **kwargs):
        ...
        def do(t):
            assert t.max() < map_tensor.shape[0], (map_tensor.shape, t.shape, t.max(), t.min())
            new_ts = map_tensor[t]
            if self.rescale_timesteps:
