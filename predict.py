...
from scripts.align import LandmarksDetector, image_align
from training.data.celeb import CelebAttrDataset
from training.data.image import ImageDataset
from training.experiments.cls import ClsModel
from training.experiments.rep import LitModel
from training.templates.templates import *
from training.templates.templates_cls import *
...
class Predictor(BasePredictor):
    ...
    def setup(self):
        ...
        state = torch.load('checkpoints/ffhq256_autoenc/last.ckpt',
                           map_location='cpu')
        ...
        state_class = torch.load('checkpoints/ffhq256_autoenc_cls/last.ckpt',
                                 map_location='cpu')
    ...
    def predict(
        ...
        image: Path = Input(
            description=
            'Input image for face manipulation. Image will be aligned and cropped, '
        ...
        for i, face_landmarks in enumerate(
            ...
            image_align(str(image), f'{self.aligned_dir}/aligned.png',
                        face_landmarks)
        ...
        if (target_class not in CelebAttrDataset.id_to_cls and
                f'No_{target_class}' in CelebAttrDataset.id_to_cls):
            cls_manipulation_amplitude = -manipulation_amplitude
        ...
        stochastic_latent = self.model.encode_stochastic(batch.to(self.device),
                                                         semantic_latent,
                                                         T=T_inv)

        cls_id = CelebAttrDataset.cls_to_id[interpreted_target_class]
        class_direction = self.classifier.classifier.weight[cls_id]
        normalized_class_direction = F.normalize(class_direction[None, :],
                                                 dim=1)

        normalized_semantic_latent = self.classifier.normalize(semantic_latent)
        normalized_manipulation_amp = cls_manipulation_amplitude * math.sqrt(
            512)
        ...
        manipulated_img = self.model.render(stochastic_latent,
                                            manipulated_semantic_latent,
                                            T=T_step)[0]
        ...
        save_image(convert2rgb(manipulated_img, adjust_scale=False),
                   str(out_path))
        model_output.append(ModelOutput(image=out_path))
        return model_output
