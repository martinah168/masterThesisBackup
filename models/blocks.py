...
from .nn import avg_pool_nd, conv_nd, linear, normalization, torch_checkpoint, zero_module
...
@dataclass
class ResBlockConfig:  # TODO REMOVE CONFIG
    channels: int
    emb_channels: int
    dropout: float
...
class ResBlock(TimestepBlock):
    ...
    def __init__(self, conf: ResBlockConfig):
        ...
        layers = [normalization(conf.channels), nn.SiLU(), conv_nd(conf.dims, conf.channels, conf.out_channels, 3, padding=1)]
...
class Downsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        self.use_conv = use_conv
        self.dims = dims
        stride = 2#(1, 2, 2)#2  # if dims != 3 else (1, 2, 2)
        if use_conv:
